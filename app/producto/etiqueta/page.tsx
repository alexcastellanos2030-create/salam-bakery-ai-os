    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6 text-xs">
      ...
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl space-y-3 text-slate-300 print:hidden">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-slate-400 mb-1 font-bold">Unidades por Empaque / Bolsa:</label>
            <input
              type="number"
              value={unidadesPorEmpaque}
              onChange={(e) => setUnidadesPorEmpaque(Number(e.target.value))}
              className="bg-slate-950 border border-slate-800 rounded p-2 text-white w-full focus:border-amber-500 focus:outline-none font-bold"
            />
          </div>
          <div>
            <label className="block text-slate-400 mb-1 font-bold">Tipo de Producto:</label>
            <input
              type="text"
              readOnly
              value={tipoProducto}
      ...
      {/* Formato de Etiqueta Térmica Imprimible */}
